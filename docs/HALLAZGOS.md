...
- **Sidetone:** Synapse convierte su escala 0–100 al headset en forma lineal (50 → 7, 31 → 4). _Fuente: captura de Synapse._ En la primera prueba en este headset **no se oyó**: el headset confirma `0x98 = 1` y `0x99 = 14` y `0x19` devuelve 14, pero el usuario no escucha su voz. Con THX instalado, Synapse además llama a `SetCaptureSidetoneLevel` de THX ([Micrófono](#micrófono)). _Fuente: `debug.log` y prueba del usuario, 2026-09-26._ **Más tarde ese día, con Synapse cerrado, sí se oyó** solo con los comandos del headset (encender, apagar y volumen). _Fuente: prueba del usuario con rzr, 2026-09-26._
...
- **Botón EQ del headset:** una pulsación corta pasa al siguiente preset de la familia: Estándar 07 → 09 → 08 → FF → 07 (Juego, Película, Música, Personalizado); Esports FA → FE → FB → FD → FC → FA (Apex, CoD, CS2, Fortnite, Valorant). Una **pulsación larga** cambia de familia y cae en el preset que recordaba. La **doble pulsación** pasa al modo Bluetooth. Cada cambio llega como evento `13` (el preset) y, si cambia la familia, también `1D` (1 Estándar, 2 Esports). _Fuente: `debug.log` y el usuario, 2026-09-26._
- **Un selector de rzr que cae en otro preset también envía el evento `13`:** al pasar de CS2 a Fortnite, el headset quedó un momento en Valorant y lo avisó (`13 [FC]`) antes del reintento. Por eso el evento solo sirve para leer el preset en ese momento: si rzr acaba de escribir, lo que vale es lo que el headset dice después de terminar la escritura. _Fuente: `debug.log`, 2026-09-26._
...
- **Caídas del enlace inalámbrico** (evento `20 [0]` y luego `20 [1]`): suelen durar ~7 s (el 2026-09-26 hubo de 1,6, 7,6, 15,1 y 7,3 s) y el dongle sigue conectado por USB (Windows no registra nada). Al usuario le pasaba igual sin ningún software, con Synapse y sin él, así que no las causa rzr: en los segundos previos rzr solo hacía lecturas. En esta PC el dongle está en un hub USB 2.1 (`05E3:0610`) al lado de otro receptor de 2,4 GHz (Keychron Link); el Wi-Fi va por 5 GHz. El usuario ya había probado antes cambiar el dongle de puerto, alejarlo y separarlo de otros receptores, sin resultado. Queda probar el headset en otro equipo. _Fuente: `conexion.log`, `debug.log` y registros de Windows, 2026-09-26._
...
| `HKLM\SOFTWARE\THX\Discovery` | `thx:sa:service = tcp://127.0.0.1:49671` y `thx:sa:status:publisher = tcp://127.0.0.1:49670`: el **servicio de THX** escucha ahí. Los puertos cambian al reinstalar THX (ver [Paquete de driver](#paquete-de-driver)). |
...
- **El dongle tiene un driver de Razer** (`rz0555dev.inf`, interfaz `MI_03`, la que usa rzr): solo incluye el HID estándar de Windows (`input.inf`, servicio `HidUsb`, sin filtros) más un coinstalador que lanza el asistente de instalación de Razer (`RzS3WizardPkgS3.exe`). Sin él, Windows usa el mismo HID genérico, así que rzr no debería notar la diferencia; si queda instalado sin Synapse, al reconectar el dongle podría ofrecer instalar Synapse. _Fuente: `C:\Windows\INF\oem141.inf` y `Get-PnpDeviceProperty`, 2026-09-26._
- **Desinstalar Synapse también desinstala THX** (observado por el usuario, y confirmado el 2026-09-26): se van los dos programas de THX, sus cuatro drivers, `VSSrv.exe` y los servicios de Razer (Game Manager, Elevation). También desaparece la interfaz de audio del headset (`MI_00`) hasta reiniciar. Queda `rz0555dev.inf`. Para quedarse con THX sin Synapse hay que respaldar antes los instaladores de `Package Cache` (se borran al desinstalar) y reinstalarlos después.
- **THX se reinstala sin Synapse** (2026-09-26): `msiexec /i` con los dos `.msi` respaldados (`wixmsi.msi` de "THX Spatial Audio USB 1532-0555" 3.2.3.0, con su `cab1.cab` al lado, y `installer.msi` de los presets 3.2.18.0), como administrador. No tienen condiciones que pidan Synapse; sus acciones corren `pnputil /add-driver … /install` con los cuatro INF. Terminan bien (código 0), `VSSrv` arranca solo y la salida del headset vuelve con los efectos de THX. Cambios que trae:
  - la salida del headset es un **dispositivo nuevo, con otro `{id}`** (el anterior desapareció);
  - los puertos de ZeroMQ cambian (esta vez `62693` el `PUB` y `62694` el `ROUTER`); se leen de `HKLM\SOFTWARE\THX\Discovery`, como ya hace rzr;
  - la salida nueva **no tiene el estado de THX** (el JSON `,6`), ni después de sonar un audio. Sin él, rzr cree que THX no está instalado.
- **Hay que reiniciar después de reinstalar THX** (2026-09-26): al reiniciar, la salida del headset vuelve a cambiar de `{id}` y esta vez sí tiene el estado `,6`. Empieza con `sequenceNumber` 1, pero con el preset "Custom" y su curva, que salen de las copias de `HKCU\Software\THX\SpatialAudio\UserState` (sobreviven a la desinstalación). El micrófono del headset tiene el efecto de THX (`VSMic`) y los puertos de ZeroMQ vuelven a `49671` y `49670`. rzr se conecta al servicio y le manda el micrófono sin cambios en el código.
...
- **Tamaños en la biblioteca de tipos** (`VSSrv` 3.2.3.0): `Get/SetMicEQGains(float[10])`, `Get/SetCurrentModeEQGains(float[31])`, `inbandMessage` `byte[4096]`, `deviceId` `ushort[80]`. _Fuente: biblioteca de tipos de `VSSrv.exe`, sesión local del 2026-09-26._
- **Dónde las guarda THX:** no aparecen en `HKCU\Software\THX`, `HKLM\SOFTWARE\THX` ni en el registro del micrófono; probablemente solo en la memoria del servicio. Synapse las guarda por perfil (`micVolumeNormalization`, `micVoiceClarity`, `ambientNoiseReduction`, `micSensitivity`, `micEqualizer`) y las manda al iniciar. _Fuente: registro de Windows y `products_1365_mw*.log`, 2026-09-26._
- **Hecho en rzr** (2026-09-26, [ADR 0007](adr/0007-microfono-en-el-perfil.md)): todas las mejoras y el EQ por `IVSSrvSettings`, confirmadas leyendo el servicio y **oídas** por el usuario escuchando su micrófono en vivo. `0x96` al headset con los bytes de Synapse (`04/96`, flag 0, largo 1, valor).
...
- **Sidetone con THX:** Synapse manda al headset `0x98`/`0x99` y además, a THX, `SetStartCaptureStatus(1)` y `SetCaptureSidetoneLevel(slider × 31,62)` (51 → 1613, 70 → 2214, 100 → 3162). Con Synapse **se oye**; solo con los comandos del headset (rzr), en la primera prueba no. **En una segunda prueba (2026-09-26, Synapse cerrado) sí se oyó solo con los comandos del headset**: `GetInputSidetoneState` = 0 y ningún proceso de Razer abierto; se encendió, se apagó y cambió de volumen desde rzr. El sidetone de Synapse no pasa por las mejoras del micrófono (lo dice la propia interfaz de Synapse).
